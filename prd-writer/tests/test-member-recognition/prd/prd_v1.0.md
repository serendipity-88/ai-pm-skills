...
当前碰一下支付场景中，用户在商家处碰手机完成支付后，会员身份需要单独出示会员码，体验断裂。数据显示，约 65% 的会员用户在碰一下支付时未出示会员身份，导致商家无法识别会员、用户无法享受会员权益。
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| enableMemberRecognition | boolean | false | 是否开启碰一下会员识别 |
...
| 指标 | 定义 | 目标值 |
|------|------|--------|
| 会员权益使用率 | 使用了会员权益的支付笔数 / 有会员身份的碰一下支付总笔数 | 12%→30%（+18pt） |
...
| 组别 | 样本 | 方案 |
|------|------|------|
| 实验组 A | 4000w | 碰一下自动识别会员 + 权益弹窗 |
| 实验组 B | 4000w | 碰一下自动识别会员 + 静默使用最优权益（不弹窗） |
