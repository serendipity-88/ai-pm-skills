...
Phase 1: 读取项目材料 → 推断 31 维度 → 写入 .prd-cache/dimensions.yaml → 输出项目理解摘要（用户确认）
         └→ 自动判断流程模式（三档）：
            ...
            H5轻量：#1=H5/小程序页面 且 #3≠复杂 且 #9≠需要状态机图 且 #6≠需要角色矩阵 且 #20≠复合交互 且 #29≠多端
...
| # | 维度 | 推断逻辑 |
|---|------|---------|
| 1 | 产品类型 | HTML→H5页面（不含Web管理后台），小程序→小程序，API→后端服务，Web管理后台→Web后台 |
...
6. **处理图片**：有截图素材时，先检测目标平台 CLI/MCP/API 是否支持图片上传或插入；支持则上传并引用，不支持则保留占位说明。无截图素材时不主动生成图片占位。
